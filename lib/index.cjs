...
class AutoRoutePlugin {
  ...
  deepReadDirSync(root, deep) {
    ...
    files.forEach((file) => {
      ...
      if (fs.statSync(absFile).isDirectory() && deep) {
...
export function getRoutes() {
  ...
  // 获取路由组件模板
  getRouterComponentTemplate(isTs, hasLayouts, indexPath, routerMode) {
...
export default function AppRouter() {
  ...
  useEffect(() => {
    if (process.env.ROUTING_MODE === 'development') {
      const pagesContext = require.context('../pages', true, /\\.(j|t)sx?$/);${hasLayouts ? `
      const layoutsContext = require.context('../layouts', true, /\\.(j|t)sx?$/);` : ""}
      pagesContext.keys().forEach(pagesContext);${hasLayouts ? `
  ...
  writeToFileAsync(filePath, fileSuffix, content) {
    try {
      fs.mkdir(filePath, { recursive: true }, (err) => {
        ...
        if (fs.existsSync(outputFile) && !this.firstRun) {
          const oldFile = fs.readFileSync(outputFile, "utf-8");
          if (oldFile === content)
            return;
        }
        this.firstRun = false;
