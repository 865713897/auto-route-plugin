...
export default function AppRouter() {
  ...
  // 生成路由文件
  async generateRoutesFile({ appData, routes }) {
  ...
  async writeToFileAsync(filePath, fileSuffix, content) {
    try {
      await promises.mkdir(filePath, { recursive: true });
      const outputFile = path.join(filePath, fileSuffix);
      await promises.access(outputFile);
      if (!this.firstRun) {
        const oldFile = await promises.readFile(outputFile, "utf-8");
        if (oldFile === content)
          return;
      }
      this.firstRun = false;
      await promises.writeFile(outputFile, content, "utf-8");
    } catch (error) {
